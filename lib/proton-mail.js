...
class ProtonMail {
  ...
  async _connect () {
    if (this._browser === undefined) {
      const rawBrowser = await puppeteer.launch({ headless: false, ...this._config.puppeteerOpts })
      const { page, _browser } = await new ExtraConfigs().instantiatePuppeteerStealthNewPage(rawBrowser, 'https://old.protonmail.com/login');
    ...
    const selectors = {
      ...
      loginButtonSelector: "div.sign-layout-main-content > form > button"
    }

    await page.waitForSelector(selectors.loginButtonSelector)

    await new Promise(r => setTimeout(r, 2000));
    await page.type(selectors.usernameSelector, this._config.username, { delay: 50 })
    await page.type(selectors.passwordSelector, this._config.password, { delay: 50 })
